...
function SuccessBanner() {
  ...
  return (
    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-center gap-3 text-emerald-300 animate-scale-in">
...
export default function OrderDetailPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-zinc-950">
      <div className="max-w-3xl mx-auto px-4 py-10 animate-pulse h-96 bg-zinc-900 border border-zinc-800 rounded-2xl" />
    </div>
  );
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 animate-fade-in">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-white">{formatCurrency(order.price)}</p>
            <Badge
              variant={
          ...
          <div className="flex items-center mb-2">
            {steps.map((step, i) => (
              <div key={step} className="flex items-center flex-1">
                <div className={`h-2.5 w-2.5 rounded-full shrink-0 transition-colors ${i <= currentStep ? "bg-indigo-500 shadow-lg shadow-indigo-500/40" : "bg-zinc-700"}`} />
                {i < steps.length - 1 && (
                  <div className={`h-0.5 flex-1 transition-colors ${i < currentStep ? "bg-indigo-500" : "bg-zinc-700"}`} />
                )}
              </div>
            ))}
          </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4 text-sm">
          <div className="min-w-0">
            ...
            <a href={order.targetUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 transition-colors flex items-center gap-1 truncate">
          ...
          <div>
            ...
            <p className="text-white font-medium">{order.anchorText}</p>
          </div>
          {order.notes && (
        ...
        {order.articleUrl && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 flex items-center gap-2">
            ...
            <span className="text-sm text-emerald-200 font-medium">Link is live!</span>
            <a href={order.articleUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-emerald-300 hover:text-emerald-200 transition-colors flex items-center gap-1 ml-auto">
      ...
      <div className="flex items-center gap-3 text-sm text-zinc-400 bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
        <Shield className="h-5 w-5 text-emerald-400 shrink-0" />
        <p>Covered by our <strong className="text-white">12-month link replacement guarantee</strong>. If the link is removed, we&apos;ll replace it within 7 days.</p>
      </div>
    </div>
    </div>
  );
