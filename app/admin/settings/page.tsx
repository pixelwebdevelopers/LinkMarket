...
export default function AdminSettingsPage() {
  ...
  if (error || !settings)
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
        <div className="max-w-3xl mx-auto px-4 py-10 text-red-700 dark:text-red-400">{error ?? "Failed to load."}</div>
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in space-y-6">
        <div>
          ...
          <p className="text-zinc-600 dark:text-zinc-400 text-sm mt-1">
            Defaults that apply across the platform. Reseller- and site-level overrides take precedence.
          </p>
        </div>

        <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 space-y-5">
          <Field
            label="Global commission (%)"
            hint="Added on top of a reseller's base price to compute the customer-facing price."
          >
            <Input
              type="number"
              step="0.5"
              ...
              onChange={(e) =>
                setSettings({ ...settings, globalCommissionPct: parseFloat(e.target.value || "0") })
              }
            />
          </Field>

          <Field
            label="Payout threshold (cents)"
            hint="Minimum reseller balance required to request a payout. 5000 = $50.00"
          >
            <Input
              type="number"
              min={0}
              ...
              onChange={(e) =>
                setSettings({ ...settings, payoutThresholdCents: parseInt(e.target.value || "0") })
              }
            />
          </Field>
