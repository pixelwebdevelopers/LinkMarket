    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 text-sm mt-1">Here&apos;s your link building overview</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            { label: "Total Orders", value: user.orders.length, icon: ShoppingCart, color: "text-indigo-700 dark:text-indigo-400 bg-indigo-500/10 border-indigo-500/20" },
            { label: "Active Orders", value: activeOrders.length, icon: TrendingUp, color: "text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
            { label: "Total Spent", value: formatCurrency(totalSpent), icon: BarChart3, color: "text-purple-700 dark:text-purple-400 bg-purple-500/10 border-purple-500/20" },
          ].map((stat) => {
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/orders" className="text-sm text-indigo-700 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors flex items-center gap-1">
                ...
                <Link key={order.id} href={`/orders/${order.id}`}>
                  <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 flex items-center gap-4 hover:border-indigo-500/30 hover:bg-zinc-100/80 dark:hover:bg-zinc-900/80 transition-all duration-150">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span className="font-bold text-zinc-900 dark:text-white text-sm">{formatCurrency(order.price)}</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {(user.role === "PUBLISHER") && user.publisher && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-zinc-900 dark:text-white text-lg">My Publisher Sites</h2>
              <Link href="/publisher" className="text-sm text-indigo-700 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors flex items-center gap-1">
                Manage <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {user.publisher.sites.map((site) => (
                <div key={site.id} className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5">
