      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-white">{getDomainFromUrl(site.url)}</h1>
            <div className="flex items-center gap-3 mt-2 text-sm text-zinc-600 dark:text-zinc-400 flex-wrap">
              ...
              <a
                ...
                className="flex items-center gap-1 text-indigo-700 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors"
              >
          ...
          {/* Metrics */}
          {metrics && (
            <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={m.label} className="text-center">
                  <p className="text-2xl font-bold text-zinc-900 dark:text-white">{m.value}</p>
                  <p className="text-xs text-zinc-500 mt-1">{m.label}</p>
                </div>
              ))}
              <div className="col-span-2 sm:col-span-4 text-xs text-zinc-500 text-center pt-3 border-t border-zinc-200 dark:border-zinc-800">
          ...
          {site.description && (
            <div>
              <h2 className="font-semibold text-zinc-900 dark:text-white mb-2">About This Site</h2>
              <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{site.description}</p>
            </div>
          )}

          <div>
            <h2 className="font-semibold text-zinc-900 dark:text-white mb-3">What&apos;s Included</h2>
          ...
          {extraNotes && (
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-sm text-amber-700 dark:text-amber-300">
              <strong className="text-amber-800 dark:text-amber-200">Publisher notes:</strong> {extraNotes}
            </div>
          )}

          {site.exampleUrl && (
            <div>
              <h2 className="font-semibold text-zinc-900 dark:text-white mb-2">Example Link</h2>
              <a
                href={site.exampleUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-indigo-700 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors flex items-center gap-1 break-all"
              >
